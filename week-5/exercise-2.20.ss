...
;the arguments that have the same even-odd parity as the first argument. For example,
...
(define (same-parity primary . set)
  (let ((parity-check (modulo primary 2)))
  (define (parity-loop result set)
    (cond ((null? set) result)
          ((= parity-check (modulo (car set) 2))
           (parity-loop (cons (car set) result) (cdr set)))
          (else (parity-loop result (cdr set)))))
  (reverse (parity-loop (list primary) set))))
